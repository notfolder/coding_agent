...
#### 2.2.3 一時停止・再開時の状態永続化

コメント検出の状態は、一時停止（pause）時に永続化し、再開（resume）時に復元します。

**永続化する状態：**
- `last_comment_ids`: 検出済みコメントIDのセット（リストとしてJSON化）
- `last_check_timestamp`: 最終チェック時刻（ISO 8601形式）

**保存場所：**
- 一時停止時: `contexts/paused/{task_uuid}/comment_detection_state.json`
- 実行中: メモリ内のみ（ファイル保存なし）

**保存形式：**
```json
{
  "last_comment_ids": ["12345", "12346", "12347"],
  "last_check_timestamp": "2024-11-28T10:30:00Z"
}
```

**一時停止時の処理フロー：**
1. PauseResumeManagerが一時停止シグナルを検出
2. CommentDetectionManagerの状態を取得
3. `comment_detection_state.json`としてpausedディレクトリに保存
4. コンテキストディレクトリをrunningからpausedに移動

**再開時の処理フロー：**
1. PauseResumeManagerがpausedディレクトリからコンテキストを復元
2. `comment_detection_state.json`が存在する場合、状態を読み込み
3. CommentDetectionManagerを復元された状態で初期化
4. 再開後は保存された`last_comment_ids`以降の新規コメントのみを検出

**復元時の注意点：**
- 状態ファイルが存在しない場合は、新規初期化と同様に全コメントを取得して`last_comment_ids`を構築
- 破損したファイルの場合もエラーログを出力し、新規初期化を実行
...
本機能は**新規の設定項目を追加せず、既存の設定を利用**します。

**デフォルト動作：**
- コメント検出機能: 有効（bot名が取得できる場合のみ）
- 検出タイミング: 毎回チェック
- ログレベル: info

**bot名の解決方法（既存設定を利用）：**

1. GitHubの場合: `github.username`を使用
2. GitLabの場合: `gitlab.username`を使用
3. いずれも設定されていない場合:
   - 警告ログを出力
   - **コメント検出機能を無効化**

**注意:** bot名が既存設定に存在しない場合、agent自身が投稿したコメントを除外できないため、機能を無効化します。これにより、無限ループの発生を防止します。
...
6. **一時停止・再開対応**: 一時停止時に状態を永続化し、再開時に復元することで継続性を確保
