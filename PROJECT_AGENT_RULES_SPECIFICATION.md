...
本仕様書は、プロジェクトディレクトリ内に配置されたLLMエージェント向けのルールファイル（AGENT.md、CLAUDE.md等）を自動的に検出し、システムプロンプトに取り込む機能を定義します。
...
| 優先順位 | ファイルパス | 説明 |
|---------|-------------|------|
| 1 | `AGENT.md` | プロジェクトルート直下のエージェント向け汎用ルールファイル |
| 2 | `CLAUDE.md` | プロジェクトルート直下のClaude特化ルールファイル |
| 3 | `.github/agents/*.agent.md` | GitHub Copilot Agents用のルールファイル（複数可） |
| 4 | `**/*.prompt.md` | プロジェクト内の任意の場所に配置されたプロンプトファイル（複数可） |
...
- **ファイル名**: 大文字・小文字を区別せず検索（`AGENT.md`、`agent.md`、`Agent.md`等）
...
#### 2.2.3 .github/agents/*.agent.md

- **配置場所**: `.github/agents/`ディレクトリ内
- **ファイル名パターン**: `*.agent.md`（任意の名前 + `.agent.md`拡張子）
- **用途**: GitHub Copilot Agents等で使用されるルールファイル
- **例**: `code-review.agent.md`、`documentation.agent.md`
- **複数ファイル対応**: ディレクトリ内のすべての`.agent.md`ファイルを検索

#### 2.2.4 **/*.prompt.md

- **配置場所**: プロジェクト内の任意のディレクトリ
- **ファイル名パターン**: `*.prompt.md`（任意の名前 + `.prompt.md`拡張子）
- **用途**: タスク固有またはコンポーネント固有のプロンプト指示
- **例**: `frontend.prompt.md`、`api/validation.prompt.md`
- **複数ファイル対応**: 再帰的にすべてのサブディレクトリを検索
...
2. 対象ファイルの存在確認（優先順位順）
   ↓
   2.1. AGENT.mdの検索（大文字小文字を区別しない）
   2.2. CLAUDE.mdの検索（大文字小文字を区別しない）
   2.3. .github/agents/*.agent.mdの検索
   2.4. **/*.prompt.mdの再帰的検索
   ↓
3. 検出されたファイルの妥当性検証
   ↓
...
1. **GitHub/GitLabタスクの場合**:
   - リポジトリ情報（owner、repo）から特定
   - MCPツールで操作可能なディレクトリをプロジェクトルートとする
   - 通常は`/home/runner/work/{repo}/{repo}`形式のパス（GitHub Actions環境の例）

2. **設定ファイルで明示的に指定する場合**:
   - `config.yaml`の`project_root`設定を使用

#### 3.2.2 プロジェクトディレクトリの検証

プロジェクトディレクトリとして有効であることを確認するための条件：

1. ディレクトリが存在すること
2. 読み取り権限があること
3. 以下のいずれかの条件を満たすこと：
   - `.git`ディレクトリが存在する
   - `package.json`、`pyproject.toml`等のプロジェクト定義ファイルが存在する
   - 設定で明示的にプロジェクトルートとして指定されている

### 3.3 ファイル検索の詳細処理

#### 3.3.1 ルートディレクトリ直下のファイル検索（AGENT.md、CLAUDE.md）

**処理内容**:
1. プロジェクトルートディレクトリの内容一覧を取得
2. ファイル名を小文字に変換して比較
3. `agent.md`または`claude.md`と一致するファイルを検出
...
#### 3.3.2 .github/agents/ディレクトリの検索

**処理内容**:
1. `.github/agents/`ディレクトリの存在を確認
2. ディレクトリが存在する場合、ディレクトリ内のファイル一覧を取得
3. `.agent.md`で終わるファイルを抽出
4. ファイル名でソートして順序を安定化

**擬似的な処理フロー**:
```
.github/agentsディレクトリが存在するか確認
存在しない場合:
    空のリストを返す
存在する場合:
    ディレクトリ内のファイル一覧を取得
    各ファイルについて:
        ファイル名が".agent.md"で終わる場合:
            リストに追加
    ファイル名でソート
    リストを返す
```

#### 3.3.3 再帰的なプロンプトファイル検索（**/*.prompt.md）

**処理内容**:
1. プロジェクトルートから再帰的にサブディレクトリを探索
2. 各ディレクトリで`.prompt.md`で終わるファイルを検出
3. 以下のディレクトリは検索対象から除外：
   - `.git`ディレクトリとそのサブディレクトリ
   - `node_modules`ディレクトリ
   - `__pycache__`ディレクトリ
   - `.venv`、`venv`、`env`等の仮想環境ディレクトリ
   - `dist`、`build`等のビルド出力ディレクトリ
   - 設定で指定された除外パターン
4. パスでソートして順序を安定化

**擬似的な処理フロー**:
```
除外ディレクトリのリストを定義: [".git", "node_modules", "__pycache__", ".venv", "venv", "env", "dist", "build"]
結果リストを初期化

再帰検索関数(現在のディレクトリ):
    現在のディレクトリ名が除外リストに含まれる場合:
        処理をスキップして戻る
    ...
    各項目について:
        項目がディレクトリの場合:
            再帰検索関数(項目のパス)を呼び出し
        項目がファイルの場合:
            ファイル名が".prompt.md"で終わる場合:
                結果リストに追加

プロジェクトルートから再帰検索関数を呼び出し
結果リストをパスでソート
```
...
| 項目 | 制限値 | 説明 |
|------|--------|------|
| `.agent.md`ファイル最大数 | 10 | `.github/agents/`内のファイル数上限 |
| `.prompt.md`ファイル最大数 | 50 | プロジェクト全体での上限 |
...
### 4.3 マークダウン構文の検出（オプション）

追加の信頼性検証として、以下のマークダウン構文要素の存在を確認できます：
...
**注意**: この検証はオプションであり、構文要素がなくてもマークダウンファイルとして扱います（プレーンテキストのルールも許容）。
...
### From: .github/agents/code-review.agent.md

[code-review.agent.mdの内容]

### From: src/api/validation.prompt.md

[validation.prompt.mdの内容]
...
システムプロンプトのトークン数が制限を超えないよう以下の対策を行います：

1. **トークン数の見積もり**（概算値、LLMモデルにより異なる場合があります）:
   - 1文字 ≒ 0.25トークン（日本語、概算）
   - 1文字 ≒ 0.4トークン（英語、概算）
   - マークダウン記号は追加トークンを消費
   - 注: これらは目安であり、使用するLLMのトークナイザーにより実際の値は異なります

2. **制限チェック**:
   - ルール追加前のシステムプロンプトのトークン数を計算
   - ルールテキストのトークン数を計算
   - 合計がLLMの最大コンテキスト長の一定割合を超える場合は警告
   - 警告閾値は設定可能（デフォルト: 20%）

3. **サイズ超過時の対処**:
   - 警告をログに記録
   - 優先順位の低いファイルから順に除外を検討
   - ユーザーへの通知（タスクコメント等）
...
- exclude_dirs: 検索から除外するディレクトリのリスト
```
...
sequenceDiagram
    ...
    FS-->>PARL: ファイル一覧
    Note over PARL: AGENT.md, CLAUDE.md を検索
...
project_agent_rules:
  ...
  # 検索対象の設定
  search:
    # ルートファイル検索の有効/無効（AGENT.md, CLAUDE.md）
    root_files: true
    
    # .github/agents/*.agent.md 検索の有効/無効
    agent_files: true
  ...
  limits:
    ...
    # トークン警告閾値（コンテキスト長に対する割合、パーセント）
    token_warning_threshold: 20
  
  # フォーマット設定
  format:
    # セクションヘッダーの言語（"en" または "ja"）
    language: "en"
    
    # ファイルパスの表示形式（"relative" または "absolute"）
...
1. **ファイル検索のテスト**
   - AGENT.mdが存在する場合に正しく検出される
   - CLAUDE.mdが存在する場合に正しく検出される
   - 大文字小文字の違いを正しく処理する
   - .github/agents/*.agent.md が正しく検出される
   - **/*.prompt.md が再帰的に検出される
   - 除外ディレクトリ内のファイルが除外される
...
## 禁止事項
- 本番環境への直接デプロイは禁止
- secretsや認証情報のハードコーディングは禁止
```

#### 10.1.2 .agent.md ファイルの推奨内容
...
#### 10.1.3 .prompt.md ファイルの推奨内容

```markdown
# API Validation Rules

このディレクトリのAPIバリデーションに関するルール：
...
2. **設定の確認**
   - `project_agent_rules.enabled`がtrueか確認
   - 除外パターンに該当していないか確認

3. **ログの確認**
   - WARNING/ERRORレベルのログを確認
   - ファイル読み込みエラーがないか確認
...
3. **安全性の確保**: サイズ制限、除外パターン、エンコーディング検証により、悪意のあるファイルや巨大ファイルから保護
