...
同一のIssue、Merge Request（MR）、Pull Request（PR）に対して、過去にコーディングエージェントが処理を行った際のコンテキスト（最終要約、計画履歴など）を引き継ぎ、処理の継続性と効率性を向上させる機能を提供します。
...
同一のTaskKey（Issue/MR/PR）を持つ過去のコンテキストが存在する場合、無条件で引き継ぎを行います。
...
**注意**: 現在のコンテキスト（current.jsonl）および全メッセージ履歴（messages.jsonl）は、トークン制限の観点から直接引き継ぎません。代わりに最終要約を使用します。
...
2. **フィルタリング**: status="completed"またはstatus="stopped"のレコードのみを対象
3. **ソート**: completed_atの降順でソート（最新のものを優先）

#### 4.1.2 検索クエリ

tasksテーブルの既存カラムを使用して検索します：
