...
| 優先度 | タスクの重要度や緊急度 | 「高優先度」 |
| スコープ | タスクの範囲と境界 | 「バックエンドのみ、UIは対象外」 |

#### 3.2.3 明確化が必要な点の特定

依頼内容に曖昧さや不明点がある場合、それを特定します：
    ...
    "ambiguities": [
      {
        ...
        "recommended_interpretation": "推奨する解釈",
        "reasoning": "推奨理由"
      }
    ],
...
2. `clarification_needed` が false、または質問が許可回数以内
3. 主要な項目（task_type, primary_goal）が特定されている
...
| エージェントルール | プロジェクト固有のルール | AGENTS.mdファイル読み取り |
...
情報収集フェーズでのツール呼び出し回数は制限されます：

| 設定項目 | デフォルト値 | 説明 |
|---------|------------|------|
| max_collection_tool_calls | 10 | 最大ツール呼び出し回数（プロジェクト規模に応じて調整推奨） |
| max_retries_per_tool | 2 | ツールあたりの最大リトライ回数 |
| collection_timeout_seconds | 120 | 収集フェーズの最大時間（複雑なプロジェクトでは延長推奨） |

**注意**: 上記のデフォルト値は中規模プロジェクト向けです。大規模プロジェクトの場合は、config.yamlで適宜調整してください。

#### 5.3.2 並列収集の検討

独立した情報収集は並列で実行可能です（設定で有効化した場合）：

- 依存関係のない収集項目を特定（収集計画のcollection_orderで依存関係を定義）
- 複数のツール呼び出しをバッチ化
- 結果を待機して統合
- 一部失敗時は成功した結果を保持し、失敗分のみリトライまたは推測

**注意**: APIレート制限があるツールでは並列収集を無効にすることを推奨します。
...
情報収集計画で定義されたデフォルト値（default_assumption）を使用します。
...
```json
{
  "assumption": {
    ...
    "verification_action": "実行フェーズでの確認方法"
  }
}
```
...
| PRE_PLANNING_MAX_TOOL_CALLS | 最大ツール呼び出し回数 | 10 |
...
| タイムアウト | 収集済み情報で継続、未収集は推測 |
| 収集回数制限超過 | 優先度の高い項目を優先して残りは推測 |
