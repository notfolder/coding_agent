...
- **ソースブランチ**: 作成した新規ブランチ
- **ターゲットブランチ**: デフォルトブランチ
- **元 Issue へのリンク**: MR/PR 本文に元 Issue へのリンクを含める

**注意**: 
- Bot ユーザーへのアサインと `coding agent` ラベルの付与は、Issue 内容の転記が完了した後に行う（4.4 参照）
- MR/PR 作成後、元 Issue に新規 MR/PR へのリンクをコメントとして投稿する（5.1 参照）
...
Issue からコメントを取得する際は、API のページネーションに注意する必要がある：

- **最新コメントの取得**: 転記最大コメント数（50件）分のコメントを、**新しいコメントから順に**取得する。Issue のコメント数が多い場合、API は複数ページに分けて結果を返すため、ページネーションを意識してコメントを取得すること
- **ページサイズ**: API のデフォルトページサイズに依存せず、効率的なページサイズを設定する
- **次ページの判定**: レスポンスヘッダーまたはレスポンスボディから次ページの有無を確認し、必要なコメント数を取得するまでループする
