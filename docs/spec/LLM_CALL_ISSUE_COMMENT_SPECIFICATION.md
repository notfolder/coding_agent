...
LLMがツール（function call）を呼び出す際、呼び出し前と呼び出し後にコメントを投稿する。

#### 4.5.1 ツール呼び出し前のコメント形式

ツール呼び出しを実行する直前に以下の形式でコメントを投稿する：
...
**呼び出し前**:
```markdown
## 🔧 ツール呼び出し - github_create_file

**引数**: {"path": "/src/new_feature.py", "content": "cl...
